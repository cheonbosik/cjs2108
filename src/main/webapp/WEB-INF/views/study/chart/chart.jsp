...
<head>
  ...
  <script>
    function chartChange() {
    	...
    	location.href='${ctp}/study2/chart?part='+part;
    }
  </script>
</head>
<body>
  ...
  <div>
    <p>학습할 차트 선택
      <select name="part" id="part" onchange="chartChange()">
        ...
        <option value="area" ${part == 'area' ? 'selected' : ''}>영역형차트</option>
      </select>
    </p>
    <hr/>
    <p>차트가 표시됩니다.</p>
    ...
    <div>
      ...
      <c:if test="${part == 'area'}"><jsp:include page="areaChart.jsp"/></c:if>
    </div>
    <hr/>
  </div>
</div>
