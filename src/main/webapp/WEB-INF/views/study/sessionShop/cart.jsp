      ...
      <c:if test="${imsiProduct != product}">		<!-- 다음 상품을 읽었을때, 같은 상품이아니면 기존상품을 출력하고 cnt는 다시 초기화(1)시킨다. 그리고, 새로운 상품을 imsiProduct에 담아준다. -->
	      <c:if test="${sw != 0}">
	        <p>
	          ...
	          <a href="${ctp}/sessionShop/cartSub?product=${imsiProduct}">(-)</a>
	          <a href="${ctp}/sessionShop/cartAdd?product=${imsiProduct}">(+)</a>
	          <a href="${ctp}/sessionShop/cartDel?product=${imsiProduct}" class="badge badge-danger">주문취소</a>
